...
- Location comes from Vercel's network geo headers by default. Country codes are formatted using `Intl.DisplayNames`; unavailable cities/regions are not guessed. Subdivision codes use the static ISO 3166 dataset where a matching name exists; otherwise they remain explicitly labeled region codes. Outside Vercel, only trust geo headers injected by your own proxy. Visitors may additionally opt into sharing precise coordinates via the browser's native geolocation permission prompt, requested once per initial visit/story view; the prompt is never silent or bypassed. If it's declined, times out, or the browser/context doesn't support it (geolocation requires a secure context), tracking falls back to the network-based location with no regression. Precise coordinates, when granted, are shown alongside the approximate location in Insights, never in place of it.
