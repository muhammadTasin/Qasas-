...
import type { getHomeStories } from '@/lib/story-data';
type Story = Awaited<ReturnType<typeof getHomeStories>>[number];
import { Clock } from 'lucide-react';

export default function StoryCard({ story }: { story: Story }) {
  return (
    <Link href={`/stories/${story.id}`} className="block group h-full touch-spring">
      <article className="story-card liquid-glass rounded-[2rem] p-8 h-full flex flex-col justify-between relative overflow-hidden hover:shadow-xl hover:shadow-emerald-900/5 transition-all duration-500">
        ...
        <div className="relative z-10 flex items-center justify-between pt-4 border-t border-white/50">
            <div className="flex items-center gap-1.5 text-xs font-sans font-medium text-ink-400">
                <Clock size={12} />
                {estimateReadTime(story.content)} min
            </div>
