...
import SiteStatsWidget from './SiteStatsWidget';
import { PenTool, User as UserIcon, BookOpen, Home, LogOut } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col pb-24 sm:pb-0 relative z-10">
      ...
      <div className="hidden sm:flex fixed top-0 left-0 right-0 z-50 justify-center pt-6 pointer-events-none">
        <nav aria-label="Main navigation" className="pointer-events-auto liquid-glass rounded-full px-1.5 py-1.5 flex items-center gap-1 shadow-lg ring-1 ring-white/50 relative">
          <div className="absolute -right-11 top-1/2 -translate-y-1/2"><ThemeToggle /></div>
          {/* Brand */}
              ...
              <div className="relative">
                ...
                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-44 rounded-2xl liquid-glass border border-white/70 shadow-xl p-2 text-sm">
                    <Link role="menuitem" href="/me" onClick={() => setMenuPath(null)} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-ink-600 hover:text-emerald-900 hover:bg-white/50 transition-colors">My stories</Link>
                    <button
                      onClick={logout}
      ...
      <div className="sm:hidden fixed bottom-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
        <nav aria-label="Mobile navigation" className="relative pointer-events-auto liquid-glass-heavy rounded-[2rem] px-6 py-3 flex items-center gap-8 shadow-2xl ring-1 ring-white/40">
          <div className="absolute -right-11 top-1/2 -translate-y-1/2"><ThemeToggle /></div>
          <Link aria-label="Home" href="/" className={`flex flex-col items-center gap-1 touch-spring ${isActive('/') ? 'text-emerald-800' : 'text-ink-400'}`}>
            <Home size={24} strokeWidth={isActive('/') ? 2.5 : 2} />
            {isActive('/') && <div className="w-1 h-1 rounded-full bg-emerald-800 mt-1"></div>}
          </Link>

          <Link aria-label={isAuthenticated ? "Write" : "Sign in to write"} href={isAuthenticated ? "/write" : "/signin"} className="relative -top-6 touch-spring">
          ...
          <button
            ...
            onClick={isAuthenticated ? toggleMenu : () => { window.location.href = "/signin"; }}
      ...
      {menuOpen && isAuthenticated && <div className="sm:hidden fixed bottom-28 right-4 z-50 w-44 rounded-2xl liquid-glass-heavy p-2 shadow-xl">
        <Link href="/me" onClick={() => setMenuPath(null)} className="block px-3 py-2 text-sm text-emerald-900">My stories</Link>
        <button onClick={logout} className="block px-3 py-2 text-sm text-emerald-900">Log out</button>
      </div>}
