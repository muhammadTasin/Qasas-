...
export default function InsightsLauncher({ storyId }: { storyId: string }) {
  ...
  return <div>
    <button type="button" onClick={open} disabled={pending} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 text-xs font-bold uppercase tracking-wide hover:bg-emerald-200 transition-colors shadow-sm">
    ...
    {data && <InsightsPanel data={data} onClose={close} />}
  </div>;
}
