...
export default function StoryEditor({ action, label, pendingLabel, publish = false, story }: {
  ...
  story?: { id: string; authorId: string; title: string; content: string; currentVersion: number };
}) {
  return <div className="max-w-4xl mx-auto pt-6 px-4 pb-20">
    ...
    {story && <StoryVersionHistory storyId={story.id} authorId={story.authorId} currentVersion={story.currentVersion} />}
    <MutationForm action={action} label={label} pendingLabel={pendingLabel} publish={publish}
      ...
      {story && <input type="hidden" name="expectedVersion" value={story.currentVersion} />}
