...
import { getVisitorIdentity } from "./analytics";
...
export const hintsSchema = z.object({ model: textHint, platform: textHint, mobile: z.boolean().optional() }).optional();
...
export async function trackingContext(request: Request, hints: z.infer<typeof hintsSchema>, scope: string) {
  ...
  if (locallyThrottled(`${scope}:${identity.visitorId || identity.ipHash}:${isAuthenticated}`)) return null;
  return { identity, metadata: trackingMetadata(request.headers, hints || {}, isAuthenticated) };
}
